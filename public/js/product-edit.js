$(function () {
    ...
    function generateVariants() {
        ...
        const isEditMode = Object.keys(existingVariantMap).length > 0;
        if (isEditMode) {
            updateVariantEngineEditMode(combinations);
        } else {
            variantIndexCounter = 0;
    ...
    function updateVariantEngineEditMode(combos) {
        ...
        combos.forEach(combo => {
            ...
            if (variantState[key]) return; // preserve existing
    ...
    setTimeout(toggleImageMode, 300);
    ...
    productForm.on('submit', async function (e) {
        ...
        if (!hasVisual) {
            const totalImages =
                (existingImages.length - removedImages.length) + allFiles.length;
        ...
        // ✅ ONLY send normal images if NOT visual mode
        if (!hasVisual) {
            allFiles.forEach(fileEntry => {
                fd.append('images[]', fileEntry.file);
            });
        }

        // ✅ ONLY send visual images if visual mode
        if (hasVisual) {
            Object.entries(newVisualFiles).forEach(([valueId, files]) => {
                files.forEach(file => {
                    fd.append(`visual_images_new[${valueId}][]`, file);
                });
            });
        }

        fd.append('visual_images_removed', JSON.stringify(removedVisualImages));
        fd.set('removed_images', removedImages.join(','));
