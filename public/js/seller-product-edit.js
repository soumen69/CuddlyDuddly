$(function () {
    ...
    const initialSubCategoryId = window.selectedSubCategoryId ?? null;
    const existingProductImages = Array.isArray(window.existingProductImages) ? window.existingProductImages : [];
    const existingAttributes = Array.isArray(window.existingAttributes) ? window.existingAttributes : [];
    const existingVariants = Array.isArray(window.existingVariants) ? window.existingVariants : [];
    const existingVisualImages = Array.isArray(window.existingVisualImages) ? window.existingVisualImages : [];
    ...
    existingVisualImages.forEach(img => {
        ...
        if (!existingVisualMap[valId]) existingVisualMap[valId] = [];
        existingVisualMap[valId].push(img);
    });
    ...
    categorySelect.on('change', function () {
        ...
        if (initialSubCategoryId && subCategorySelect.find(`option[value="${initialSubCategoryId}"]`).length) {
            subCategorySelect.val(String(initialSubCategoryId)).trigger('change');
        }
    });

    subCategorySelect.on('change', function () {
        const isEditMode = Object.keys(existingVariantMap).length > 0;
        if (!isEditMode) {
        ...
        $.get(attributeUrl).done(function (data) {
            ...
            <div class="variant-check-zone mb-3">
                <label class="block">${attr.name}</label>`;
                    attr.values.forEach(val => {
                        const checked = existingAttributes.some(a => a.attribute_value_id == val.id) ? 'checked' : '';
                        html += `
            ...
            `;
                ...
                setTimeout(() => {
                    if (isEditMode && Object.keys(variantState).length === 0) {
                        hydrateVariantStateFromExisting();
                    }
    ...
    function hydrateVariantStateFromExisting() {
        existingVariantMap = existingVariantMap || {};
        const tbody = getVariantBody();

        Object.values(existingVariantMap).forEach(v => {
            const key = (v.values || []).map(val => parseInt(val.id)).sort((a, b) => a - b).join('-');
            const label = (v.values || []).map(val => val.value).join(' / ');
            const valueIds = (v.values || []).map(val => parseInt(val.id));
    ...
    function getVariantBody() {
        ...
        if (!tbody.length) {
            $('#variantTable').html('<table class="w-full"><tbody></tbody></table>');
            tbody = $('#variantTable tbody');
        }
        return tbody;
    }

    function createRow(index, key, label, state, valueIds) {
        const checked = state.enabled ? 'checked' : '';
        const hiddenValues = valueIds.map(v => (
        ...
        return $(`
            <tr data-key="${key}">
                <td>
                    <input type="hidden" name="variants[${index}][enabled]" value="0">
                    <input type="checkbox" name="variants[${index}][enabled]" value="1" ${checked}>
                </td>
                <td>${label}${hiddenValues}</td>
                <td><input type="text" name="variants[${index}][sku]" class="form-control" value="${state.sku ?? ''}"></td>
                <td><input type="number" step="0.01" name="variants[${index}][price]" class="form-control" value="${state.price ?? ''}"></td>
                <td><input type="number" name="variants[${index}][stock]" class="form-control" value="${state.stock ?? ''}"></td>
            </tr>
        `);
    }
    ...
    $(document).on('click', '.visual-dropzone .preview-remove', function () {
        ...
        } else if (typeof index !== 'undefined' && visualImageFiles[value]) {
    ...
    const updatePreviews = () => {
        ...
        previews.classList.toggle('hidden', !allFiles.length);
    };
    ...
    const simulateUpload = () => {
        ...
        const timer = setInterval(() => {
            uploaded++;
            bar.style.width = `${(uploaded / allFiles.length) * 100}%`;
    ...
    previews.addEventListener('click', e => {
        ...
        if (existingId) {
            removedProductImageIds.push(String(existingId));
    ...
    function initCreateVisualDropzone(block, valueId) {
        ...
        function render() {
            ...
            const existing = (existingVisualMap[valueId] || [])
                .filter(img => !removedImages.includes(String(img.id)));
            const has = existing.length > 0 || visualImageFiles[valueId].length > 0;
            if (has) {
    ...
    productForm.on('submit', function (e) {
        ...
        const existingVisualCounts = Object.fromEntries(
            Object.entries(existingVisualMap).map(([attrId, images]) => [
                attrId,
                images.filter(img => !removedImages.includes(String(img.id))).length
            ])
        );
        const hasVisualVariants =
            Object.keys(existingVisualCounts).some(attrId => existingVisualCounts[attrId] > 0) ||
            Object.keys(visualImageFiles).some(attrId => (visualImageFiles[attrId] || []).length > 0);

        if (hasVisualVariants) {
            const visualGroups = new Set([
                ...Object.keys(existingVisualCounts),
                ...Object.keys(visualImageFiles)
            ]);

            for (let attrId of visualGroups) {
                const totalForGroup =
                    (existingVisualCounts[attrId] || 0) + ((visualImageFiles[attrId] || []).length);

                if (totalForGroup < 3) {
                    showError('Each visual variant must have at least 3 images.');
                    return;
                }
            }
        } else {
            const keptExisting = existingProductImages.filter(
                img => !removedProductImageIds.includes(String(img.id))
            ).length;
            const totalProductImages = keptExisting + allFiles.length;

            if (totalProductImages < 3) {
                showError('At least 3 product images are required.');
                return;
            }
        }
        if (uploading) {
        ...
        existingProductImages.forEach(img => {
            if (!removedProductImageIds.includes(String(img.id))) {
                fd.append('existing_images[]', img.id);
            }
        });

        if (Array.isArray(removedImages) && removedImages.length) {
            fd.append('visual_images_removed', JSON.stringify(removedImages));
        }
        ...
        if (hasVisualVariants) {
            ...
            const existingVisualTotal = Object.values(existingVisualCounts)
                .reduce((sum, count) => sum + count, 0);

            if (totalFiles + existingVisualTotal === 0) {
            ...
            .then(async res => {
                ...
                if (!res.ok) {
                    const firstError = data && data.errors
                        ? ([]).concat(...Object.values(data.errors))[0]
                        : null;
                    const msg = firstError || data.message || 'Validation error';
                    showError(msg);
                    throw new Error(msg);
                }
