    ...
    <div class="product-accordion-content overflow-hidden transition-[max-height] duration-500 ease-in-out p-0">
        <div class="px-6 py-6">
            ...
            <!-- Media Upload -->
                ...
                <div id="dropZone"
                    class="w-full border border-black/20 bg-white rounded-sm p-6 flex flex-col items-center justify-center cursor-pointer hover:bg-blue-100 transition-all min-h-[180px]">
                    <!-- ICON + TEXT -->
                    ...
                    <div id="imagePreviews" class="flex flex-wrap gap-3 mt-4 {{ isset($product) && $product->images->count() ? '' : 'hidden' }}">
                        @if(isset($product))
                            @foreach($product->images as $img)
                                <div class="preview-item existing" data-id="{{ $img->id }}">
                                    ...
                                    <button type="button" class="preview-remove"
                                        data-existing-id="{{ $img->id }}" title="Remove">
                    ...
                    <div id="uploadProgress" class="w-full mt-4 hidden">
                        <div class="w-full bg-gray-200 rounded-full h-2">
                            <div class="progress-bar h-2 rounded-full bg-black" style="width:0%"></div>
                        </div>
                        <small class="text-sm">
                            Uploading:
